...
# System prompt for thinking mode (R1-style reasoning)
R1_SYSTEM_PROMPT = """
...
def load_model(model_path, load_in_8bit=True, thinking_mode=False):
    """Load InternVL3 model with optional 8-bit quantization and thinking mode"""
    ...
    if thinking_mode:
        ...
        print("Thinking mode system prompt applied!")
...
def extract_answer(response, num_choices=4):
    ...
    # If thinking mode is enabled, extract answer from text after </think> tag
    ...
    # Try to find A, B, C, or D in the answer text
    response_upper = answer_text.upper().strip()
...
def run_inference(model, tokenizer, questions, base_path, num_frames=8, thinking_mode=False, batch_size=1):
    ...
    # Configure generation settings based on thinking mode
    if thinking_mode:
...
def main():
    ...
    parser.add_argument(
        ...
        choices=['OpenGVLab/InternVL3_5-4B-Instruct', 'OpenGVLab/InternVL3_5-4B', 'OpenGVLab/InternVL3_5-8B'],
    ...
    if args.thinking:
        print(f"  - Generation: do_sample=True, temperature=0.6, max_tokens=1024")
    print(f"{'='*80}\n")
